...
can_manage = app_state.current_user_role() in permissions.CAN_MANAGE_SITES_AND_SAMPLING
if not can_manage:
    st.info("Your role has read-only access to sites, sampling events and samples.")
...
if can_manage:
    with st.expander("Register a new site", expanded=False):
        ...
        if submitted and app_state.check_permission(permissions.CAN_MANAGE_SITES_AND_SAMPLING, "create a site"):
            try:
                with get_session() as session:
...
if can_manage:
    with st.expander("Register a new sampling event", expanded=False):
        ...
        if submitted_event and app_state.check_permission(
            permissions.CAN_MANAGE_SITES_AND_SAMPLING, "create a sampling event"
        ):
            try:
...
if can_manage:
    with st.expander("Register a new sample", expanded=False):
        ...
        if submitted_sample and app_state.check_permission(
            permissions.CAN_MANAGE_SITES_AND_SAMPLING, "create a sample"
        ):
            try:
                with get_session() as session:
