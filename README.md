...
- The registry/auth app (`Home.py` + `pages/`) is separate from the
  synthetic-data demo (`streamlit_dashboard.py`) — they are two independent
  Streamlit apps in this repo for now. Connecting real omics uploads and
  model predictions to specific registered samples is a future increment.
...
pytest tests/test_auth.py tests/test_models.py tests/test_registry.py -v
```

## Registry app (`Home.py`)

A separate Streamlit app (independent of `streamlit_dashboard.py`) exposing
the persistence layer above through a UI: registration/login, and
project → site → sampling event → sample registration, each scoped to the
logged-in user.
...
Pages call `ai_wasteguard.registry` and `ai_wasteguard.auth` rather than
querying the database directly (`app_state.py` is the thin Streamlit-session
glue). Verified end-to-end in a real browser: register → log in → create
project → create site → create sampling event → create sample.
